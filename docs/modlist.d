#!/usr/bin/env rdmd
/**
 * Copied and modified from https://github.com/dlang/dlang.org/blob/master/modlist.d
...
struct Tree {
	...
	void dump(string[] pkgs) {
		if(leaves.empty) {
			...
			if(pkgMod) {
				writefln("%s$(PACKAGE $(MODULE%s %-(%s, %), %s),",
					indent(pkgs.length),
					pkgs.length + 1,
					pkgs,
					name);
			} else {
				writefln("%s$(PACKAGE $(PACKAGE_NAME %s),",
					indent(pkgs.length),
					name);
...
int main(string[] args) {
	...
	void add(string path, ref Tree tree) {
		auto files = dirEntries(path, "*.d", SpanMode.depth)
			...
			// relative to path
			.map!(de =>
				de.name.chompPrefix(path).chompPrefix(dirSeparator))
			// std/digest/sha.d => std.digest.sha
			.map!(n => n.chomp(".d").replace(dirSeparator, "."))
			.filter!included;
